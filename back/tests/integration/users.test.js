...
describe('api/users', () => {
  ...
  describe('GET /current', () => {
    it('should return 401 if user is not logged in', async () => {
      const res = await request(appServer).get(`${route}/current`);
  ...
  describe('POST /login', () => {
    it('should return 400 if user logges in without login and password', async () => {
      const res = await request(appServer).post(`${route}/login`).send({});
      expect(res.status).toBe(400);
      expect(res.body).toMatchObject({
        email: 'Поле Email обязательно к заполнению',
        password: 'Поле Пароль обязательно к заполнению',
  ...
  describe('GET /all', () => {
    it('should return 200 and user object if user object is in the users array', async () => {
      ...
      expect(res.body).toContainEqual({
        ...
        email: 'emv3@narod.ru',
        password:
          '$2a$10$D3CMlipIz5RMEMMjW5SWDOUhId6bOrGKHOtAMDf4OGvVeH5uY5eoe',
  ...
  describe('POST /register', () => {
    it('should return 400 if user registers with short name', async () => {
      ...
      expect(res.status).toBe(400);
      expect(res.body).toMatchObject({
        name: 'Имя не должно быть короче 2 и длиннее 30 знаков',
      });
    });
  });
  ...
  describe('POST /register', () => {
    it('should return 400 if user registers with long name', async () => {
      const res = await request(appServer).post(`${route}/register`).send({
        name:
          'agfhbfdgnjfdghjndfghmjghjmdghmkdghmdghmdghmdghmdghmdghmdghmdghmdghmdghm',
        email: 'emv5@narod.ru',
        password: '555555',
        password2: '555555',
      });
  ...
  describe('POST /register', () => {
    it('should return 400 if user registers without password2', async () => {
      ...
      expect(res.status).toBe(400);
      expect(res.body).toMatchObject({
        password2: 'Поле Подтверждение пароля обяъательно к заполнению',
      });
    });
  });
