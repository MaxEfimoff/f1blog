      ...
      .then(profile => {
        NewsItem.findById(req.params.id)
          .then(newsItem => {
            ...
            if(newsItem.likes.filter(like => like.user.toString() === req.user.id).length > 0) {
              return res.status(400).json({ alreadyliked: 'User already liked this news article' })
            }
          ...
          }).catch(err => res.status(404).json({ postnotfound: 'News article not found' }));
     })
  }
