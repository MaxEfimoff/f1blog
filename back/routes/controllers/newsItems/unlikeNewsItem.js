...
const unlikeNewsItem = (req, res) => {
  try {
    ...
    if (
      newsItem.likes.filter((like) => like.profile === profile)
        .length === 0
    ) {
      return res
        .status(400)
        .json({ status: 'fail', notliked: 'You have not yet liked this news article' });
    }
  ...
  newsItem.save().then((newsItem) => res.json(newsItem));
    ...
    res
      .status(404)
      .json({ status: 'fail', cannotunlikenewsitem: 'Can not unlike this news article' });
  }
};
